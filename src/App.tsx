      ...
      <main className="mx-auto w-full max-w-5xl flex-1 p-4 sm:p-6">
        ...
        >
          {view === 'archive' ? (
            ...
            <div className="grid grid-cols-2 grid-rows-2 overflow-hidden rounded-xl border border-gray-300 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
              {QUADRANTS.map((q, i) => (
                <QuadrantPanel
                  key={q.id}
                  quadrant={q.id}
                  tasks={activeTasks.filter((t) => t.quadrant === q.id)}
                  onAdd={() => openAdd(q.id)}
                  onEdit={openEdit}
                  onToggle={toggleDone}
            ...
            <LaneView tasks={activeTasks} onAdd={openAdd} onEdit={openEdit} onToggle={toggleDone} onArchive={archiveTask} activeTask={activeTask} hideCompleted={hideCompleted} activeTag={activeTag} />
          )}
